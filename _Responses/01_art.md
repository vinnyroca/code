---
layout: page
title: Art and Technology
permalink: /Responses/01_art/
---
# Art and Technology

## Description

Readings for this assignment can be found [here](https://scrippscollege.box.com/s/3je1kmbfik4jxdl2dhqeoajkj3sh1owr) (Check the class Discord for password.)

**Don’t wait until the weekend before discussion to begin engaging with the sources for our response. During the weeks between discussions,  move through the sources slowly**

**This response is due by 5pm the day before our discussion.**

For this response, do the following:

1. Watch History of the Future, Art & Technology from 1965
   
   <iframe width="560" height="315" src="https://www.youtube.com/embed/mHox98NFU3o?si=kbba45I8SwVgfJn3" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
1. Read the Introduction to *Glitch Feminism* by Legacy Russell
2. Cyberfeminism:
    - Either using the *Cyberfeminism Index* in the classroom, or using the website https://cyberfeminismindex.com/ , find an entry that strikes you.  You may want to use the collections section as a starting point.
    - After finding an entry, conduct some brief research on the topic to find information on the way the entry thinks about art and technology.

## Submit

In the Discord channel for Response 1, post either a 500 word or 3 min video response considering the following questions (these are not strict questions to answer, but rather starting points for thinking about these sources):

- How does Casey Reas’ understanding of the relationship between art and technology align with or differ from the views of Legacy Russell and the Cyberfeminism entry you researched?
- In which way do these sources understanding of technology differ?
- From these sources, which views of technology and art do you relate to?
- Did any of these sources provide you with a different perspective on the relationship between art, technology and code?
